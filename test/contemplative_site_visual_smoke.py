...
ROUTES = (
    {"name": "home", "path": "/", "root": ".contemplative-home", "current": None},
    {
        "name": "research",
        "path": "/publications/",
        "root": ".research-page",
    ...
    {"name": "notes", "path": "/blog/", "root": ".notes-index", "current": "/blog/"},
    {
        "name": "gallery",
        "path": "/gallery/",
        "root": ".gallery-page",
    ...
    {"name": "cv", "path": "/cv/", "root": ".contemplative-cv", "current": "/cv/"},
)
...
def _assert_shared_surface(page: Page, route_specification: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        ...
        route_specification: Mapping with ``root`` and optional ``current`` href.
    ...
    Side effects:
        Moves the pointer once to activate the authored effect, then reads DOM
        and computed styles without persistent mutation.
    """
    ...
    assert measurements["navCount"] == 5, measurements
...
def _assert_route_content(page: Page, route_name: str, viewport_name: str) -> None:
    ...
    if route_name == "home":
        ...
        assert page.locator(".article-entry").count() == 3
        assert page.locator(".gallery-home-preview").count() == 0
        assert page.locator(".article-entry a[href*='/blog/']").count() == 3
    ...
    if route_name == "notes":
        assert page.locator(".notes-index-entry").count() == 5
        assert page.locator(".notes-reading-time, .notes-entry-meta").count() >= 5
        return
...
def _inspect_route(
    ...
    Args:
        ...
        route_specification: Route name, URL path, root selector, and current href.
    ...
    measurements = _assert_shared_surface(page, route_specification)
    _assert_route_content(page, route_specification["name"], viewport["name"])
...
def main() -> int:
    """Run the complete six-route, two-viewport production browser matrix.

    Returns:
        Process status ``0`` after all twelve visits pass. Any assertion or
        Playwright error propagates and produces a non-zero process status.

    Side effects:
        Launches system Chrome, performs twelve local HTTP visits, writes twelve
        PNG screenshots, and prints one concise evidence line per visit.
    """
    # Parse machine paths once and allocate a single shared browser process.
    ...
    with sync_playwright() as playwright:
        ...
        for viewport in VIEWPORTS:
            for route_specification in ROUTES:
                results.append(
                    _inspect_route(browser, arguments, route_specification, viewport)
                )
        browser.close()
